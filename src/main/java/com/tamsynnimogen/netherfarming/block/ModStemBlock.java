...
public class ModStemBlock extends StemBlock {
    ...
    protected boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return state.matchesBlock(ModBlocks.TILLED_SOUL_SOIL.get());
    }
        ...
        if (worldIn.getLightSubtracted(pos, 0) >= 9) {
            ...
            if (net.minecraftforge.common.ForgeHooks.onCropsGrowPre(worldIn, pos, state, random.nextInt((int)(25.0F / f) + 1) == 0)) {
                ...
                if (i < 7) {
                    ...
                    if (worldIn.isAirBlock(blockpos) && (blockstate.canSustainPlant(worldIn, blockpos.down(), Direction.UP, this) || block == ModBlocks.TILLED_SOUL_SOIL.get() || block == ModBlocks.FERTILE_SOUL_SOIL.get() || block == Blocks.SOUL_SOIL || block == Blocks.DIRT || block == Blocks.COARSE_DIRT || block == Blocks.PODZOL || block == Blocks.GRASS_BLOCK)) {
    ...
    protected Item getSeedItem() {
        if (this.crop == ModBlocks.GLOOM_SQUASH.get()) {
            return ModItems.GLOOM_SQUASH_SEEDS.get();
        } else {
            return this.crop == ModBlocks.HELLKIN.get() ? ModItems.HELLKIN_SEEDS.get() : null;
        }
    }
