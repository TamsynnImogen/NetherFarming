...
public class ModAttachedStemBlock extends AttachedStemBlock {
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
        return !facingState.matchesBlock(this.grownFruit) && facing == stateIn.get(FACING) ? this.grownFruit.getStem().getDefaultState().with(StemBlock.AGE, Integer.valueOf(7)) : super.updatePostPlacement(stateIn, facing, facingState, worldIn, currentPos, facingPos);
    }
    ...
    protected Item getSeeds() {
        if (this.grownFruit == ModBlocks.GLOOM_SQUASH.get()) {
            return ModItems.GLOOM_SQUASH_SEEDS.get();
        } else {
            return this.grownFruit == ModBlocks.HELLKIN.get() ? ModItems.HELLKIN_SEEDS.get() : Items.AIR;
        }
    }
